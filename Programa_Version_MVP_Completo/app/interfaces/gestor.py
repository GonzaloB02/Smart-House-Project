...
def agregar_dispositivo():
    while True:
        ...
        if nombre is None:
            print(f'Acción cancelada.\n')
            return 

        if DispositivoDAO.buscar_existencia_nombre(nombre) and DispositivoDAO.dispositivo_activo(nombre) != False:
        ...
        DispositivoDAO.re_activar(nombre) if DispositivoDAO.dispositivo_activo(nombre) == False else DispositivoDAO.crear_dispositivo(nombre, tipo_dispositivo_eleccion, id_estado=2, activo=1)
...
def agregar_automatizacion():
    while True:
        ...
        if AutomatizacionDAO.buscar_existencia_nombre(nombre) and AutomatizacionDAO.automatizacion_activa(nombre) != False:
            print(f'La automatizacion {nombre} ya se encuentra registrada.\n')
            return
        ...
        AutomatizacionDAO.re_activar_automatizacion(nombre, dispositivo_indice, hora_inicio, hora_fin, estado_eleccion,) if AutomatizacionDAO.automatizacion_activa(nombre) == False else AutomatizacionDAO.crear_automatizacion(nombre, dispositivo_indice, hora_inicio, hora_fin, estado_eleccion)
...
def buscar_automatizacion():
    nombre = normalizar_nombre(input('\nIngrese el nombre de la automatizacion que desea buscar: ')).strip()
    
    if AutomatizacionDAO.buscar_existencia_nombre(nombre):
        AutomatizacionDAO.imprimir_busqueda(nombre) if AutomatizacionDAO.automatizacion_activa(nombre) != False else print('Automatizacion no encontrada.\n')
        return
    else:
        print('Automatizacion no encontrada.\n')
...
def modificar_rol_usuario():
    while True:
        ...
        if rol_eleccion is None:
            print(f'Acción cancelada.\n')
            return 

        if RolDAO.obtener_id_rol(rol_eleccion) is None:
