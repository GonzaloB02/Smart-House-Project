...
class DispositivoDAO:
    ...
    def obtener_todo_activo_dispositivo():
        filas = fetch_all('''
                        ...
                        WHERE activo = 1''')
        return [Dispositivo(*fila) for fila in filas]
    
    def obtener_id_dispositivo(id_dispositivo):
        filas = fetch_one('''
                        SELECT id_dispositivo, nombre_dispositivo, id_tipo_dispositivo, id_estado, activo
                        FROM Dispositivo
    ...
    def re_activar(nombre):
        ejecutar_consulta('''
                        UPDATE Dispositivo
                        SET id_estado = ?, activo = ?
                        WHERE nombre_dispositivo = ?''', (2, 1, nombre))
    ...
    def dispositivo_activo(nombre):
        ...
        return fila[0] if fila[0] == 0 else None
